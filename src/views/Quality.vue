...
const name = ref("")
const spec = ref("")
const searchForm = reactive({ name, spec })
                ...
                <!-- <div v-if="!userStore.isReadOnly" style="display: flex; justify-content: space-between">
                    <span>
                        ...
                        <ExportButton v-model="selectedRows" :headers="headers" :tableData="tableData.value"
                            fileName="产品库存信息.xlsx" :filterFunction="filterExportData" buttonLabel="导出" />
                    </span>
                    <span>
                        <el-button
          ...
          "> -->
            ...
            { required: true, message: '请选择工段名称', trigger: 'blur' },
            {
                ...
                message: '长度必须在1-30之间', trigger: 'blur'
                        ...
                        <SearchComponent :hide-title=true :wNo="100" :key="renderKey" search-title="工段名称"
                            :search-content=addform.section field="section" @search="search"
            ...
            { required: true, message: '请选择产品名称', trigger: 'blur' }]">
                                <DialogSearch :key="renderKey" :wNo="100" search-title="产品名称"
                                    :searchContent=stockform.name field="name" @search="dialogSearchSuggestion"
                                    :data="stockform" database="products" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
                            <el-form-item label="规格型号" prop="spec" :rules="[
            ...
            { required: true, message: '请上传入库凭证', trigger: 'blur' }]">
                ...
                <!-- 出库弹框 -->
                <DialogComponent ref="stockOutDialog" :form="stockform" dialog-title="产品出库"
                    :refreshFunc="getDataFromAPI" :confirm-func="addProductOperationAPI" @dialogClose="dialogClose"
                    ...
                    <el-row>
                        <el-col :span="12">
                            <el-form-item label="产品名称" prop="name">
                                <DialogSearch :key="renderKey" :wNo="100" search-title="产品名称"
                                    :searchContent=stockform.name field="name" @search="dialogSearchSuggestion"
                                    :data="stockform" database="products/operation" />
                            </el-form-item>
                        </el-col>
                        <el-col :span="12">
            ...
            { required: true, message: '请选择物料批次', trigger: 'blur' }]">
                        <DialogSearch :key="renderKey" :wNo="100" search-title="产品批次" :searchContent=stockform.batch
            ...
            { required: true, message: '请上传出库凭证', trigger: 'blur' }]">
                ...
                <el-table :data="tableData.value" @selection-change="handleSelectionChange"
                    ...
                    <el-table-column prop="qualified" label="质检数量" align="center" />
                    <el-table-column prop="discarded" label="不合格数量" align="center" />
