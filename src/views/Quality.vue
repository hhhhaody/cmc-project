...
const local = (receipt) => {
    ...
    const receiptMap = JSON.parse(receipt);
    imageUrls.value = Object.values(receiptMap).map(url => ("src/assets/images/" + url + '.png'));
    currentIndex.value = 0;
    dialogVisible.value = true;
}
...
watch(
    ...
    ([newImageUrls, newIndex], [oldImageUrls, oldIndex]) => {

        // console.log('New imageUrls:', newImageUrls);
        // console.log('Old imageUrls:', oldImageUrls);

        // console.log('New currentIndex:', newIndex);
        // console.log('Old currentIndex:', oldIndex);

        console.log(newImageUrls[newIndex]);


        if (newImageUrls[newIndex].endsWith('.pdf')) {
            console.log('pdf');

            // dialogPdfUrl.value = file.url!
            pdfUrl = ref(newImageUrls[newIndex]);
            pdf.value = true
            ...
            <el-main style="overflow: hidden">
                ...
                <div>
                    <SearchComponent :key="renderKey" search-title="产品名称" :searchContent=name ref="search1" field="name"
                        @search="search" @edit="edit" database="products" :data="searchForm" />
          ...
          ">
            ...
            { required: true, message: '请上传操作凭证', trigger: 'blur' }]">
                ...
                <el-table :data="tableData.value" @selection-change="handleSelectionChange"
                    ...
                    <!-- <el-table-column prop="batch" label="产品编号" align="center" min-width="120vh" /> -->
                    <el-table-column prop="name" label="产品名称" align="center" />
                    <el-table-column prop="spec" label="产品编号" align="center" min-width="200vh" />
                    <el-table-column prop="inspectionNo" label="质检编号" align="center" min-width="50vh" />
                    ...
                    <el-table-column prop="receipt" label="凭证" align="center" min-width="40vh">
                        ...
                        <template #default="scope">
                            <el-button class="inline_button" @click="local(scope.row.receipt)">
                                详情
                            </el-button>
                        </template>
                    </el-table-column>
    ...
    <el-dialog v-model="dialogVisible" style="width: fit-content;border-radius: 1vh;">
        ...
        <div v-else style="display: flex;">
            <div v-if="currentIndex === 0">
                <img v-if="imageUrls[0]" :src="imageUrls[0]" alt="图片1"
                    style="width: 100%; max-width: 80vh; margin-right: 10px;" />
                <img v-if="imageUrls[1]" :src="imageUrls[1]" alt="图片2" style="width: 100%; max-width: 80vh;" />
            </div>
            <img v-else w-full :src="imageUrls[currentIndex - 1]" alt="无图片" class="image" />
