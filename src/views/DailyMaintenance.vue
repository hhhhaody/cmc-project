...
const getDataFromAPI = async () => {
    const res = await getFacilityAPI(currentPage.value, pageSize.value, name.value, spec.value, section.value, status.value, maintenanceman.value, true);
...
const getMaintenancePlanByID = async (id) => {
    ...
    if (res.code === 1) {
        ...
        stockform.prevDailyTime = res.data.prevDailyTime
        stockform.serialNo = res.data.serialNo
        stockform.status = res.data.status
        stockform.prevInfo = res.data.prevInfo
        stockform.prevMaintenanceman = res.data.prevMaintenanceman
            ...
            <el-main style="overflow: hidden">
                ...
                <div style="display: flex; justify-content: space-between">
                    ...
                    <span>
                        ...
                        <!-- <el-button>
                            <RouterLink to="/maintenancePlan">设备维护计划</RouterLink>
                        </el-button> -->
                        <!-- <el-button
          ...
          ">
            ...
            <el-form :model="item" @submit.prevent label-position=right>
              ...
              <el-form-item label="维护内容" prop="info">
                ...
                <el-table :data="tableData.value" @selection-change="handleSelectionChange"
                    ...
                    <el-table-column prop="prevDailyTime" label="上次维护日期" align="center" min-width="100vh">
                        <template #default="scope">
                            {{ scope.row.prevDailyTime ? scope.row.prevDailyTime.substring(0, 10) : '' }}
                        </template>
                    </el-table-column>
            ...
            <template #default="scope">
              {{ scope.row.completeTime ? scope.row.completeTime.substring(0, 10) : '' }}
                    ...
                    <el-table-column prop="prevMaintenanceman" label="维护人员" align="center" />
