...
const getDataFromAPI = async () => {
    ...
    if (props.station === '总装工作站') {
        res = await getProductAmountAPI('型钢切割工作站');

        res.data = [
            { name: '4F', 总装工作站: 6, 总装工作站2: 11, spec: '模块' },
            { name: '5F', 总装工作站: 2, 总装工作站2: 11, spec: '模块' },
            { name: '6F', 总装工作站: 8, 总装工作站2: 11, spec: '模块' },
            { name: '7F', 总装工作站: 0, 总装工作站2: 11, spec: '模块' },
            { name: '8F', 总装工作站: 0, 总装工作站2: 11, spec: '模块' },
        ...
        res.data = [
            ...
            { name: '(F1.1#.4F)-D#模块箱', 墙板生产线: 2, 墙板生产线2: 7, spec: '墙板' },
            { name: '(F1.1#.4F)-E#模块箱', 墙板生产线: 5, 墙板生产线2: 5, spec: '墙板' },
            { name: '(F1.1#.4F)-F#模块箱', 墙板生产线: 0, 墙板生产线2: 2, spec: '墙板' },
            { name: '(F1.1#.4F)-G#模块箱', 墙板生产线: 0, 墙板生产线2: 4, spec: '墙板' },
            { name: '(F1.1#.4F)-H#模块箱', 墙板生产线: 0, 墙板生产线2: 4, spec: '墙板' },
            { name: '(F1.1#.4F)-I#模块箱', 墙板生产线: 0, 墙板生产线2: 6, spec: '墙板' },
            { name: '(F1.1#.4F)-J#模块箱', 墙板生产线: 0, 墙板生产线2: 4, spec: '墙板' },
            { name: '(F1.1#.4F)-K#模块箱', 墙板生产线: 0, 墙板生产线2: 2, spec: '墙板' },
            { name: '(F1.1#.4F)-L#模块箱', 墙板生产线: 0, 墙板生产线2: 5, spec: '墙板' }
        ]
    ...
    BarChart.setOption({
        ...
        tooltip: {
            ...
            formatter(params) { // 自定义提示信息的内容
                ...
                return ` ${params.seriesName}：${params.data.value}<br> ${params.name}所需${params.data.spec}`; // 返回自定义的提示信息
                console.log(params);
            },
        },
        xAxis: {
